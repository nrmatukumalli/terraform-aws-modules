...
locals {
  create_dhcp_options = var.domain.name != null ? true : false
}
...
resource "aws_vpc_dhcp_options" "this" {
  ...
  domain_name_servers  = var.domain.servers
  ntp_servers          = var.domain.ntp_servers
  netbios_name_servers = var.domain.netbios_name_servers
  netbios_node_type    = 2
